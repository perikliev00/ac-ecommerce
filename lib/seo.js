...
function buildSeo(opts) {
  ...
  const locale = (req && req.locale) || 'bg';
  ...
  const result = {
    ...
    robotsMeta,
    themeColor,
    ogLocale: locale === 'en' ? 'en_US' : locale === 'de' ? 'de_DE' : locale === 'ru' ? 'ru_RU' : 'bg_BG',
...
function buildProductJsonLd(req, product, productDetails) {
  const baseUrl = getBaseUrl(req);
  const productUrl = baseUrl + localizePath('/product/' + (product.id || product._id), (req && req.locale) || 'bg');
...
/**
 * Build FAQPage JSON-LD from list of { question, answer }.
...
/**
 * Merge LocalBusiness + optional BreadcrumbList + optional Product + optional FAQ into jsonLd array.
 */
function mergeJsonLd(req, options = {}) {
  ...
  if (options.faq && options.faq.length) {
    const faqSchema = buildFaqJsonLd(options.faq);
    if (faqSchema) arr.push(faqSchema);
  }
  return arr;
}
